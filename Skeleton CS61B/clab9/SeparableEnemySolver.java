...
public class SeparableEnemySolver {
    ...
    public boolean isSeparable() {
        for (String label: g.labels()) {
            if (isOddCircle(label, label, new ArrayList<>(),true)) {
                return false;
            }
        }
        return true;
    }
    private boolean isOddCircle(String label, String label1,ArrayList<String> parents, boolean odd) {
        parents.add(label1);

        for (String neighborlabel : g.neighbors(label1)) {
            if (odd==true && neighborlabel==label){
                return true;
            }
            if (g.neighbors(neighborlabel).size()==1){
                continue;
            }
            else if (parents.contains(neighborlabel) ){
                continue;
            }
            else{
            return isOddCircle(label,neighborlabel,parents,!odd);}
        }
        return false;
    }
