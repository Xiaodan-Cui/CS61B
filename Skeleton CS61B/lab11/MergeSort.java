...
public class MergeSort {
    ...
    private static <Item extends Comparable> Queue<Queue<Item>>
        ...
        // Your code here!
        Queue<Queue<Item>> SingelItemQueues = new Queue<>();
        for (Item i :items) {
            Queue<Item> qi=new Queue<>();
            qi.enqueue(i);
            SingelItemQueues.enqueue(qi);
        }
        return SingelItemQueues;
    }
    ...
    public static <Item extends Comparable> Queue<Item> mergeSort(
            Queue<Item> items) {
        // Your code here!
        Queue<Queue<Item>> SIQs=makeSingleItemQueues(items);
        while (SIQs.size()>1){
            Queue<Queue<Item>> DIQs=new Queue<>();
            while(!SIQs.isEmpty()){
            Queue<Item> q1=SIQs.dequeue();
            if (!SIQs.isEmpty()){
            Queue<Item> q2=SIQs.dequeue();
            Queue<Item> DIQ=mergeSortedQueues(q1,q2);
            DIQs.enqueue(DIQ);}
            else{
                DIQs.enqueue(q1);
            }
